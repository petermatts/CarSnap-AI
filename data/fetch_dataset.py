...
def download_and_extract_stanford_cars():
    ...
    ds.save_to_disk("STANFORD")


def download_and_extract_dmv_cars():
    dataset_dir = Path(__file__).parent / DatasetNames.DMV.value
    if not dataset_dir.exists(): dataset_dir.mkdir(parents=True)
...
def download_and_extract_vmmrdb():
    dataset_dir = Path(__file__).parent / DatasetNames.VMMR.value
    if not dataset_dir.exists(): dataset_dir.mkdir(parents=True)

    data_file = dataset_dir / "data.zip"
    if not os.path.exists(data_file):
        url = URL("https://www.dropbox.com/scl/fi/3r3a0oz71nlvhqhu3tocf/VMMRdb.zip?rlkey=e5bb62ae88x2pzxnsggobxpe0&e=2&dl=1")
        download(url, data_file)
    else:
        print("File exists. Skipping download.")
...
if __name__ == "__main__":
    ...
    ds.add_argument("--stanford", action="store_const", dest="ds", const=DatasetNames.STANFORD.value, help="Downloads the Stanford Cars dataset")
    ds.add_argument("--dmv", action="store_const", dest="ds", const=DatasetNames.DMV.value, help="Downloads the DMV Cars dataset")
    ds.add_argument("--vmmr", action="store_const", dest="ds", const=DatasetNames.VMMR.value, help="Downloads the VMMR dataset")
